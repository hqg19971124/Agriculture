...
class ModifiedStudentModel_skip(nn.Module):
    def __init__(self):
        ...
        # self.multi_scale1 = MultiScaleModule_dilated(16, 16)
        # self.multi_scale2 = MultiScaleModule_dilated(32,32)
    ...
    def forward(self, x):
        ...
        # x = self.multi_scale(residual)

        x = self.relu1(residual)
        x = self.maxpool1(x)

        # x = self.multi_scale1(x)# 多尺度放在第一个卷积层后
        x = self.conv2(x)  # Depthwise separable convolution
        x = self.relu2(x)
        x = self.maxpool2(x)
